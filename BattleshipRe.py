# BattleShipRe.py
# A Python module that creates the game Battleship and lets the player play it
...
# Define Sea_spaces class. Takes window and self as parameters. Creates the
# battleship board. Methods include get_sea_spaces, place_ship_computer, and 
#place_ship.
class Sea_spaces:

    
    # Define constructor. Takes window, self, and player as parameter. Sets up
    # the battleship board.
    def __init__(self, window, player):
        
        # If player is human exectue if body
        if player == 'human':
            # Create empty list
            self.sea_space_list = []
            # Store window
            self.win = window
            # Loop over range 27 - 37
        ...
        # If player is computer execute elif body
    ...
    # Define get_sea_spaces method. Takes self as parameter. Returns 
    #sea_space_list
    def get_sea_spaces(self):
         
        # Return sea_space_list
        return self.sea_space_list

    
    # Define place_ship_computer method. Takes self as parameter. Makes
    #computer player place ships automatically.
    def place_ship_computer(self):
        ...
        for i in [1, 2, 2, 3, 4]:
            ...
            # Draw text in window
            text.draw(self.win)
            # Set difference variable to be 0
            difference = 0
            # Set first and second click variables to be false
            first_click = False
            second_click = False
            
            # While difference does not equal to 1 execute while body
            while difference != i:
                # While first_click equals to false execute while body
                while first_click == False:
                    # Generate random integers in range 2 - 11
                    x = random.randint(2,11)
                    y = random.randint(2,11)
                    # Create point with integers generated
                    p = Point(x, y)
                    # Set first_click to be True
                    first_click = True
                    # Generate random number in range 1 -2
                    rand = random.randint(1,2)
                    # If random integer is equal 1 execute if body
                    if rand == 1:
                        # If x coordinate of point plus i is less than or equal
                        #to 11 execute if body
                        if p.getX() + i <= 11:
                            # Create s value
                            s = p.getX() + i
                            # Create second point
                            p2 = Point(s, p.getY())
                            # Set second_click to be True
                            second_click = True
                        # If x coordinate plus i is greater than 11 set
                        #first_click to False
                        else:
                            first_click = False
                    
                    # If random integer is not 1 execute else body    
                    else:
                        # If y coordinate plus i  is less than or equal to 11
                        #execute if body
                        if p.getY() + i <= 11:
                            # Create d value
                            d = p.getY() + i
                            ...
                            # Set second_click to be True
                            second_click = True
                        # If y coordinate plus i is greater than 11 set
                        #first_click to be False
                        else:
                            first_click = False
                ...
                # Difference of x coordinates or y coordinates is equal to i
                #execute if body
                if (difference_x == i) or (difference_y == i):
                    # Create empty list and set difference to be i
                    values = []
                    difference = i
                    # If y values are the same execute if body
                    if y1 - y2 == 0:
                        # if x1 is less than x2 execute if body
                        if x1 < x2:
                            ...
                            for x_value in range(x1, x2 + 1):
                                ...
                                for space in self.sea_space_list:
                                    #If space at Point(x_value, y1) is clicked
                                    # execute if body
                                    if space.clicked(Point(x_value, y1)):
                                        #Append ship status of space to values
                                        #list
                                        values.append(space.get_ship_status())
                            # If True is in values list proceed
                            if True in values:
                                # Set difference to be 0
                                difference = 0
                                # Set first and second click variables to False
                                first_click = False
                                second_click = False
                            # If True is not in values list execute else body
                            else:
                                # Loop over coordinates in x1 to x2 + 1 range
                                for x_value in range(x1, x2 + 1):
                                    # Loop over spaces in sea_space_list
                                    for space in self.sea_space_list:
                                        #If space at Point(x_value, y1) is
                                        #clicked execute if body
                                        if space.clicked(Point(x_value, y1)):
                                            # Update ship
                                            space.update_ship()
                        # x2 is smaller than x1 execute else body
                        else:
                            ...
                            # If True not in values list execute else body
                                ...
                                # Loop voer coordinates in x2 to x1 + 1 range
                                for x_value in range(x2, x1 + 1):
                                    # Loop over spaces in sea_space_list
                                    for space in self.sea_space_list:
                                        ...
                                        if space.clicked(Point(x_value, y1)):
                                            # Update ship
                                            space.update_ship()
                    # If x coordinates are equal execute elif body
                    elif x1 - x2 == 0:
                        # y1 less than y2 execute if bodu
                        if y1 < y2:
                            # Loop over coordinates in y1 to y2 + 1 range
                            for y_value in range(y1, y2 + 1):
                                # Loop over spaces in sea_space_list
                                for space in self.sea_space_list:
                                    #If space at Point(x1, y_value) clicked
                                    #execute if bodu
                                    if space.clicked(Point(x1, y_value)):
                                        # Append ship status to values list
                                        values.append(space.get_ship_status())
                        ...
                        # If y2 less than y1 execute else body
                            ...
                            for y_value in range(y2, y1 + 1):
                                ...
                                for space in self.sea_space_list:
                                    ...
                                    #execute if body
                                    if space.clicked(Point(x1, y_value)):
                                        # Append ship status to value list
                                        values.append(space.get_ship_status())
                            # If True in values list execute if body
                            if True in values:
                                # Set difference to 0
                                ...
                                for y_value in range(y2, y1 + 1):
                                    ...
                                    for space in self.sea_space_list:
                                        ...
                                        if space.clicked(Point(x1, y_value)):
                                            # Update ship
                                            space.update_ship()
                    # If neither y or x coordinates are equal execute else body
                    else:
                        # Set difference to 0
                        difference = 0
                        # Set first and second click variables to False
    ...
    # Define place_ship method. Takes self as parameter. Asks the user for
    #where he/she wants to place their ships, and places them.
    def place_ship(self):
